...
def discrete(x):
    ...
    if x >= 100:
        ...
        delta = math.log(100) / k
        print 
        return int(round(math.log(x+1)/delta))


def generate_data(file_list, cluster, cluster_list):
    output = open('../traindata_old_mul' + cluster + '.csv','w')
    output_real = open('../realdata_mul' + cluster + '.csv','w')
        ...
        for index,num in enumerate(tmp):
            if index > 0:
                if tmp[index-1] == 0:
                    ...
                    line.append( discrete((((tmp[index]+0.0)/tmp[index-1]) - 1)*100) )
